namespace SequenceEqualExceptIntersectLINQMethodsExamples
{
    public class LinqSamples : ViewModelBase
    {
        ...
        {
            //NumberComparer numberComparer = new();

            List<int> oddNumbersList = new() { 1, 3, 5, 7, 9 };
            List<int> evenNumbersList = new() { 2, 4, 6, 8, 10 };

            // SequenceEqual Method Syntax
            var result = oddNumbersList.SequenceEqual(evenNumbersList);
            //var result = evenNumbersList.SequenceEqual(oddNumbersList, numberComparer);
            ...
            List<Product> list2 = new()
      {
        ...
        public List<Product> ExceptByMethod()
        {
            ...
            // The list of colors to exclude from the list
            List<string> colors = new() { "Purple", "Blue" };

            // ExceptBy() Method Syntax
            productList = products.ExceptBy(colors, p => p.Color).ToList();
        ...
        public List<Product> IntersectByExample()
        {
            List<Product> productlist;
            // Load all products
            List<Product> products = ProductRepository.GetAll();
            ...
            // IntersectBy Method Syntax
            productlist = products.DistinctBy(p => p.ProductID).ToList();
